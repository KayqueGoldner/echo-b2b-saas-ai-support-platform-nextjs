...
export const addFile = action({
  ...
  handler: async (ctx, args) => {
    ...
    const { entryId, created } = await rag.add(ctx, {
      // IMPORTANT: search space must be defined. We cannot search across namespaces,
      ...
      contentHash: await contentHashFromArrayBuffer(bytes), // to avoid re-inserting if the same file content hasn't changed
    });
